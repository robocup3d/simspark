...
the center of the robot's torso. That means that the observed information of
visible objects changes if the robot is moving or turning.
...
The first parameter is the name of the axis. Table \ref{table:perceptorNames}
shows a list of all available hinge joints of the Soccerbot robot. Table
\ref{tab:nao-conf} those of the NAO robot. The second parameter contains
the speed of change in angle of the joint. So a value of 3 means 3 degrees per cycle.
%Klaus: is that true? degree per cycle?
\begin{itemize}
...
The first parameter is the name of the axis. Table \ref{table:perceptorNames} shows a list of all
available universal joints of the Socerbot robot. The NAO robot does not have
universal joints. The second and third parameter contain the speed of change in
angle of the two joints. The order of the joints is the same as in the name.
\begin{itemize}
...
respect to the coordinate system of figure \ref{fig:pitch}. The rot value allows
to define the rotation angle of the player. Zero degrees points to positive x axis, 90
...
%Joschka: no, the Soccerbot has a say effector in the current version. It is restricted to 8 bytes msgs.
